 ...
 *
 * Usage: npx tsx dev/warp-preview.ts [dump.json] [out-prefix] [frac] [expand] [margin]
 */
...
const W = mode === 'box'
  ? buildBoxExpandWarp(samples, box, { frac, expand, marginFrac })
  : buildSepBoxWarp(samples, box, { alpha: 0.8, maxScale: 8 }, { frac, expand, marginFrac });
...
console.log(`wrote ${outPrefix}.png — ${wn.length} nodes, ${edges.length} edges, ${boxes.length} box(es), frac=${frac} expand=${expand} margin=${marginFrac}`);
