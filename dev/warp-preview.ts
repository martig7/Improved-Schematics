 ...
 * the dense box(es) being expanded. Lets us tune frac/userMult/margin in seconds.
 *
 * Usage: npx tsx dev/warp-preview.ts [dump.json] [out-prefix] [frac] [userMult] [margin] [maxGrowth]
 */
...
import { buildDemandBoxWarp, buildSepDemandBoxWarp, findDenseBoxes } from '../src/render/layout/densityBoxWarp';
import type { BoxGraph } from '../src/render/layout/densityBoxWarp';
...
const userMult = Number(process.argv[5] ?? 4);
const marginFrac = Number(process.argv[6] ?? 3);
const maxGrowth = Number(process.argv[7] ?? 1);
...
const graph: BoxGraph = { nodes: nodesRaw, edges };
// cellFromMedLen mirrors renderGeographic's octi cellSize estimate closely
// enough for a tuning preview (divisor 1.6, floor 12px) — exact match isn't
// needed here, the preview is for eyeballing the warp shape.
const cellFromMedLen = (m: number) => Math.max(12, m / 1.6);
...
const demandOpts = { frac, marginFrac, userMult, maxGrowth, cellFromMedLen };
const W = mode === 'box'
  ? buildDemandBoxWarp(samples, graph, box, demandOpts).warp
  : buildSepDemandBoxWarp(samples, graph, box, { alpha: 0.8, maxScale: 8 }, demandOpts).warp;
...
console.log(`wrote ${outPrefix}.png — ${wn.length} nodes, ${edges.length} edges, ${boxes.length} box(es), frac=${frac} userMult=${userMult} margin=${marginFrac} maxGrowth=${maxGrowth}`);
