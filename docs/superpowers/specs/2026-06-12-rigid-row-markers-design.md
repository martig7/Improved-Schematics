...
- **R1 (Shape by construction, with stated precision).** Every rendered marker is an octilinear polygon up to two sub-pixel tolerances: (i) the parallel-row join admits lateral offsets < 0.75 px, so a joined chord of length $g$ may tilt up to $\arcsin(0.75/g)$ off-axis (≈4° at the shortest observed joins — invisible at marker scale, and the dots remain exactly on their lanes); (ii) SVG coordinates are emitted at 0.1 px quantization. The octilinearity gate therefore uses the length-aware bar $\max(1°, \arcsin(0.85/\text{len}))$ per segment. Markers whose stations are moved by the post-solve collision-slide passes have their derived corners RECOMPUTED: the new corner is the intersection of the lines through the slid boundary dots along the OLD leg directions (octilinear by construction, since leg directions are solver axes); if the legs are near-parallel the corner is cleared instead. Clearing alone is unsound for bent markers on non-parallel lanes (the chord's off-axis residual is invariant under equal-arc slides — proven at SEA mn177). The only intentionally non-octilinear marker is the axis-aligned mega box.
