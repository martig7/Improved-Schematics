...
| `src/render/transfers.ts` (+ test) | 2026-07-06 | none (feature removed) | Walk-transfer connector brackets between nearby station groups not joined by a route edge. Rendered zero connectors across the entire dump corpus (close stations are already route-linked, so every candidate pair is excluded by `renderTransferConnectors`), so removal is byte-identical. |
