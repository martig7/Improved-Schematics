...
interface DetailInsetProps {
  ...
  theme: { land: string; water: string; green: string; stationFill: string; stationStroke: string; place?: string };
...
export function DetailInset({
  ...
  useEffect(() => {
    ...
    const drawResim = (subPre: SmoothedPrecomputed, selFrame: Rect | null) => {
      const out = drawSmoothedSchematic(subPre, { showLabels, showStations, stationDesign, landmass, landmassDetail, dark, theme: { ...theme, lineWidth: 4, stationRadius: 3 }, placeColor: theme.place });
  ...
  }, [sel.box, getMainPre, getCacheKey, baseSvg, showStations, showLabels, stationDesign, landmass, landmassDetail, dark, theme, position, buildInput, applyLabelScale]);
