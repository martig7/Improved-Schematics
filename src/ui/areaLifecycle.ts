...
// or keep the on-screen areas. Getting this wrong has two failure modes. One is clearing
// on a spurious re-render or same-fingerprint regenerate, so areas vanish though the
// layout is identical. The other is resurrecting stale areas onto a different layout.
//
...
export function decideAreaAction<T>(i: AreaDecisionInput<T>): AreaAction<T> {
  ...
  // (4) Same key, or the very first paint. The layout is unchanged (a re-render that does
  //     not alter it), so keep what's on screen.
  ...
  // (3) Any other key change, such as a genuinely different smoothed fingerprint or a
  //     switch INTO a non-smoothed mode. The on-screen boxes are stale (wrong-layout
  //     coords), so clear.
  return { kind: 'clear' };
}
