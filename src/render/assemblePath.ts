...
/** Whether a lane polyline emits visible ink. */
export function laneHasExtent(pts: readonly Pixel[]): boolean {
  return pts.length >= 2 && laneExtent(pts) >= 1e-6;
}
...
export function assembleDByLine(args: AssembleArgs): Map<string, string[]> {
  ...
  const emitPiece = (d: string[], pts: Pixel[], cur: Pixel | null): Pixel | null => {
    ...
    if (!laneHasExtent(pts)) return cur;
