// Cheap, stable digest of the inputs that determine the octi layout. It is the
// would-be cache key for the deferred Plan B precompute cache (docs/cache-plan-B.md).
//
// Its current use is a `[fp]` diagnostic logged in the Generate path, to confirm
// the game emits identical ids/coords for an unchanged network across a
// save then reload (Plan B §0, the prerequisite that decides whether a
// localStorage cache can ever hit). It returns per-component sub-hashes too, so
...
const SCHEMA = 27; // bump to bust all fingerprints when the renderer's inputs change
//
...
// game inputs stay unchanged. Bumping busts both the main caches and the
// detail-inset sub-pres, which are both keyed on this fingerprint, so they
// re-simulate with the new geometry. The `stationSplit` render option is
// included in the fingerprint because it changes the default node model.
...
export function fingerprintInputs(input: FingerprintInput): Fingerprint {
  // Stations. Only constructed ones count, matching how graph.ts filters on
  ...
  // stationRadius, which are draw-time and applied fresh on restore).
  ...
  // Geography token. Presence plus coarse content (NOT bbox, which drifts).
